...
review_android = select(review_android, -c(Language, Author, Title, TranslatedTitle, TranslatedReview, ReplyDate, DeveloperReply, User, Device, DeviceType, Tags, Notes, Link))
...
#test
review_android$Review = textclean::replace_contraction(review_android$Review) 
review_android$Review = lemmatize_strings(review_android$Review)
data("stop_words")
review_android$Review = removeWords(review_android$Review, stopwords("English"))
head(review_android$Review)

#review_android$Review <- stem_words(review_android$Review)
#review_android$Review <- lemmatize_words(review_android$Review)
#review_android$Review <- stem_strings(review_android$Review)
#review_android$Review <- lemmatize_strings(review_android$Review)

#Data Cleaning Phase 1:
review_android$Review <- replace_contraction(review_android$Review) #Convert contractions to base
review_android$Review <- tolower(review_android$Review)# convert to lowercase
removeNumPunct <- function(x) gsub("[^[:alpha:][:space:]]*", "", x)#remove puncuation function
review_android$Review <- removePunctuation(review_android$Review)# remove punctuation
review_android$Review <- gsub("[[:punct:]]", "",review_android$Review)
review_android$Review <- removeNumbers(review_android$Review)# remove numbers
review_android$Review <- removeNumPunct(review_android$Review)# remove non latin characters except space

# Turn dataframe to tibble, number the reviews
tbl_reviews <- as_tibble(review_android) %>%
...
corpus_review=Corpus(VectorSource(review_android$Review))
...
tidy_reviewshc <- removeSparseTerms(corpus_review, sparse = 0.9)
